...
export function AdminUsersTab() {
  ...
  return (
    <Card>
      <CardHeader
        title="Utilisateurs"
        subtitle="Gérez les comptes et leurs rôles. Le premier compte créé est administrateur. Stockage local (navigateur)."
      />
      <CardBody className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Input
            ...
            className="max-w-xs"
          />
          <div className="text-xs text-[var(--rp-muted)]">{accounts.length} compte(s)</div>
        </div>
                  ...
                  <div className="md:col-span-5 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-[var(--rp-muted)]">
                        @{acc.username} · inscrit le {new Date(acc.createdAt).toLocaleDateString("fr-FR")}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-4">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setMsg(
                          r.ok
                            ? { ok: true, text: `Rôle de @${acc.username} mis à jour.` }
                  ...
                  <div className="md:col-span-3 md:text-right">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setMsg(
                          r.ok
                            ? { ok: true, text: `Compte @${acc.username} supprimé.` }
                            : { ok: false, text: r.error },
                        );
                      }}
                    >
        ...
        <p className="text-[11px] leading-relaxed text-[var(--rp-muted)]">
          Les permissions de chaque rôle se configurent dans l’onglet{" "}
          <span className="font-semibold">Rôles &amp; permissions</span>. Vous pouvez aussi y créer
          de nouveaux grades (Helper, Streamer, VIP…).
        </p>
      </CardBody>
    </Card>
  );
